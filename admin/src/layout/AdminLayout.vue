<template>
  <div class="flex h-screen overflow-hidden bg-[#0A0D14]">
    <!-- Sidebar -->
    <aside class="w-64 glass flex flex-col border-r border-border-glass">
      <div class="p-6 border-b border-border-glass">
        <h1 class="text-xl font-black gradient-text tracking-tight">GameShop</h1>
        <p class="text-xs text-text-muted mt-1 flex items-center gap-1.5">
          <span class="w-1.5 h-1.5 rounded-full bg-neon-cyan animate-pulse-glow inline-block"></span>
          Admin Panel
        </p>
      </div>
      ...
      <div class="p-3 border-t border-border-glass">
        <button @click="handleLogout" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-text-secondary hover:text-red-400 hover:bg-red-500/10 w-full transition-all duration-300 ease-spring">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
...
const navItems = [
  ...
  { path: '/orders', label: 'Orders', icon: 'shopping' },
